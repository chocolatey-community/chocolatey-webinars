function New-LenovoMetapackage {
    <#
    .SYNOPSIS
    ...
    Param(
        ...
        [ValidateScript({
                foreach ($item in $_) {
                    ...
                    elseif ($item -is [string] -and $item -match '\.nupkg$') {
                        continue
                    }
                    else {
                        throw "Each dependency must be either a hashtable or a string ending in '.nupkg'"
                    }
                }
                $true
            })]
        ...
        if ($Dependency -is [Hashtable]) {
            $dependencyArgs.Add('Dependency',$Dependency)
        }
        else {
            $dependencyArgs.Add('Nupkg',$Dependency)
        }

        New-Dependency @dependencyArgs
    }
}