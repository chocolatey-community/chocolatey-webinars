<#
    ...
    .EXAMPLE
        ...
        # Installs everything.
#>
[CmdletBinding(DefaultParameterSetName = 'NoServers')]
param(
    ...
    [Parameter(ParameterSetName = "RepositoryOnly", Mandatory)]
    [Parameter(ParameterSetName = "RepositoryAndCCM", Mandatory)]
    [Alias('Url')]
    ...
    [Parameter(ParameterSetName = "RepositoryOnly")]
    [Parameter(ParameterSetName = "RepositoryAndCCM")]
    ...
    [Parameter(ParameterSetName = "RepositoryAndCCM", Mandatory)]
    [Parameter(ParameterSetName = "CCMOnly", Mandatory)]
    ...
    [Parameter(ParameterSetName = "RepositoryAndCCM")]
    [Parameter(ParameterSetName = "CCMOnly")]
    [Alias('ClientSalt')]
    ...
    [Parameter(ParameterSetName = "NoServers", Mandatory)]
    [Parameter(ParameterSetName = "CCMOnly", Mandatory)]
    $LicensePath,
...
if ($RepositoryUrl) {
    ...
    $NupkgUrl = if (-not $ChocolateyVersion) {
        ...
        $QueryUrl = switch ($RepositoryType) {
            'nexus-repository' { 
                Write-Verbose 'Using a Sonatype Nexus repository'
                (($RepositoryUrl -replace '/index\.json$'), "v3/registration/Chocolatey/index.json") -join '/' 
            }
            'proget' { 
                Write-Verbose 'Using an Inedo ProGet repository'
                (($RepositoryUrl -replace '/index\.json$'), "registrations/Chocolatey/index.json") -join '/' 
            }
        }
...
$InstallScriptUrl = if ($RepositoryUrl) {
    ...
    switch ($RepositoryType) {
        'nexus-repository' {
            ...
            Write-Verbose "Script found at : $RepositoryUrl"
        }

        'proget' {
            $RepositoryUrl -replace '\/nuget\/(?<RepositoryName>.+)\/v3\/(index.json)?$', '/endpoints/choco-install/content/ChocolateyInstall.ps1'
        }
    }
...
Write-Verbose "Using script from : $InstallScriptUrl"
